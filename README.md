...
VPN在技术上要更加底层一些，安装好VPN之后可以在网络中心里面，看到这个VPN的网卡设置，说明VPN是通过虚拟一张网卡，然后对主机上的网络传输数据进行全局加密传输，在启动的时候，会首先与VPN服务器建立点对点的连接，所以启动的时候速度会比较慢，但是一旦建立连接之后速度就很快了。而SSR只是相当于主机上的一个应用程序，还是建立在TCP基础之上的一个协议，所以启动的时候，只需要在本地启动程序开启端口即可，所以启动会更快，通常这个端口是1080http端口与socks自定义端口，而通过浏览器上网是http协议，所以实际上浏览器是首先将请求转发到本地SSR的1080端口，然后再由SSR转发到服务器的。还有一个需要说明的是SSR的"全局模式"，这里并不能对主机上的所有请求代理，而是根据本地的一个pac的json文件进行选择性代理，这个pac文件通常是被防火长城墙了的网站域名，SSR就可以根据判断如果是这些域名则走代理，否则直接访问，这样可以加快用户访问网络的速度。
