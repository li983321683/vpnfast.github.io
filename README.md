# 2019中国最好用的翻墙软件VPN推荐，翻墙到国外必备VPN，11月更新
...
ExpressVPN可以说的上是业界的领导者了，凭借其领先的加密技术和超快的网络访问速度，成为了翻墙到国外最好用的VPN，绝对是VPN排名第一
...
NordVPN之前在中国的速度表现还算不错，包年套餐比较划算，但是最近翻墙效果非常差，不建议使用
...
VPN在技术上要更加底层一些，安装号VPN之后可以在网络中心里面，看到这个VPN的网卡设置，说明VPN是通过虚拟一张网卡，然后对主机上网络传输数据进行"全局"加密传输，在启动的时候，会首先与VPN服务器建立点对点的连接，所以启动的时候速度会比较慢，但是一旦建立连接之后速度就很快了。而SSR只是相当于主机上的一个应用程序，还是建立在TCP基础之上的一个协议，所以启动的时候，只需要在本地启动程序开启端口即可，所以启动会更快，通常这个端口是1080http端口与socks自定义端口，而通过浏览器上网是http协议，所以实际上浏览器是首先将请求转发到本地SSR的1080端口，然后再由SSR转发到服务器的。还有一个需要说明的是SSR的"全局模式"，这里并不能对主机上的所有请求代理，而是根据本地的一个pac的json文件进行选择性代理，这个pac文件通常是被防火长城墙了的网站域名，SSR就可以根据判断如果是这些域名则走代理，否则直接访问，这样可以加快用户访问网络的速度。
...
现在由于国内政策收紧，墙的力度更加严格了，普通的VPN流量很容易被墙识破，一旦某些IP大面积发现特征流量，这个IP段基本就不能用了，这也是自己搭建Shadowsocks存在的问题，一旦IP被封通常需要付费购买IP。针对突破中国网络的审查，VPN商家需要投入很多开发成本和运营成本，并且每年墙的检测复杂程度也在提升，所以这需要VPN商家有强大的底子支撑，才能够长期维持VPN的稳定可靠。本文的VPN推荐都是经过我自己严格测试了的，而不是网上其他推荐一样，自己都没用用过就推荐给别人。我自己平时也是经常几家换着在用，最少要做两手准备，以防梯子在墙外的尴尬。
...
现在国内的审查制度是非常严格的，之前在写的一篇VPN推荐知乎文章都被删了，更何况是售卖VPN的商家更加会严厉打击。而且对于普通的VPN来说，虽然报文内容是加密的，但是报文协议却很容一检测到。如果某些小商家用了一些开源协议的话，被墙检测到的几率会大大增加。对于一线商家来说，他们有资源在现有协议上做一些二次开发，从而绕过墙的检测。今年国庆期间，使用自己搭建的Shadowsocks几乎全部被封了，因为有太多人在某几家VPS服务商上面都搭建了翻墙软件，导致一旦某个IP的流量被检测到Shadowsocks的指纹，这个IP段的用户都会被封，国内到国外的TCP和UDP数据包均不能正常发送。如果只是为了翻墙，而不是对翻墙技术有兴趣想去实践的话，我们还是推荐购买付费VPN更加省事儿，免得特殊时期又会面临没有梯子的尴尬。付费VPN之所以不会面临自建Shadowsocks的问题，是因为他们通常在全球范围内都租用了大量的VPN服务器，就算几台IP被封也能够快速切换到其他服务器，不会影响用户的正常使用。
